// use the fs module the asynchrounous way through callback functions
const fs = require("fs");
const mars = "mars.txt";
const venus = "venus.txt";
const jupiter = "jupiter.txt";

// JS code in this case is unblocking, meaning JS can move to the next line while
// an fs operation is waiting for the callback to be executed.
...
async function f() {
  ...
  let data = fs.promises.readFile(jupiter, "utf-8");
  console.log(`${jupiter} content: ${data}`);
  await fs.promises.appendFile(jupiter, "\nIt is so fun!");
  data = fs.promises.readFile(jupiter, "utf-8");
  console.log(`${jupiter} content: ${data}`);
}
